...
$packageContentDirectoriesFile = $ExecutionContext.SessionState.Path.GetUnresolvedProviderPathFromPSPath("package_content_directories.txt")
...
$packageContentDirectories = @()

# read package content file, id it exists
if (Test-Path $packageContentFile)
{
	$packageContentDirectories += Get-Content $packageContentFile | ForEach-Object (Join-Path $rootDir -ChildPath $_)
}
...
foreach ($packageContentDirectories in $packageContentDirectories)
{
	# write progress message
